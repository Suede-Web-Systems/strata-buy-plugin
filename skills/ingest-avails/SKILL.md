...
3. **Always call out the phantom-inventory number.** Seller files routinely carry rate periods on days a program does not air. Spots placed there are **silently dropped by Strata on import**. Tell the buyer how many periods are phantom and that the optimizer automatically excludes them. If the file already carries spots, report the predicted-survival totals the parser prints.
